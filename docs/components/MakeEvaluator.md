...
- **Purpose**: Produce BuildFacts: evaluated rules/commands, inferred compile actions, custom commands, `ProjectGlobals`, and `UnknownConstruct` entries where evaluation cannot map behavior.
...
- **Behavior details**:
  ...
  - Conditional handling for `ifeq`/`ifneq`/`ifdef`/`ifndef`; unsupported functions/conditionals emit WARN + `UnknownConstruct` (category `make_function`/`conditional_logic`) with normalized form (e.g., `eval(call(...))`).
